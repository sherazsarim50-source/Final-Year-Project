<?php
	...
	if(isset($_POST) & !empty($_POST)){
		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			$size1 = $_FILES['productimage1']['size'];
			$type = $_FILES['productimage']['type'];
			$type1 = $_FILES['productimage1']['type'];
			$tmp_name = $_FILES['productimage']['tmp_name'];
			$tmp_name1 = $_FILES['productimage1']['tmp_name'];
			$max_size = 10000000;
			$extension = substr($name, strpos($name, '.') + 1);
			$extension1 = substr($name1, strpos($name1, '.') + 1);

			if(isset($name) && isset($name1)){
					// if(isset($name1) && !empty($name1)){
						if(($extension == "jpg" || $extension == "jpeg") && ($extension1 == "jpg" || $extension1 == "jpeg")&& $type == "image/jpeg" && $size<=$max_size && $type1 == "image/jpeg" && $size1<=$max_size ){
				// if(($extension == "jpg" || $extension == "jpeg") && $type == "image/jpeg" && $size<=$max_size){

					$location = "uploads/";
					$filepath = $location.$name;
					$filepath1 = $location.$name1;
				
					if(move_uploaded_file($tmp_name, $filepath) && move_uploaded_file($tmp_name1, $filepath1)){
							// if(move_uploaded_file($tmp_name1, $filepath1)){
			...
			$filepath1 = $_POST['filepath1'];
		}	

		$sql = "UPDATE products SET name='$prodname', description='$description', catid='$category', price='$price', thumb='$filepath', thumb1='$filepath1' WHERE id = $id";
	...
	<div class="content-blog">
		...
		<?php if(isset($smsg)){ ?><div class="alert alert-success" role="alert"> <?php echo $smsg; ?> </div><?php } ?>
			...
			<form method="post" enctype="multipart/form-data">
			  ...
			  <input type="hidden" name="filepath1" value="<?php echo $r['thumb1']; ?>">
			  ...
			  <div class="form-group">
			    ...
			    <br>
			    	...
			    	<a href="ajax/delprodimg.php?id=<?php echo $r['id']; ?>">Delete Image</a>
			    <?php }else{ ?>
			  ...
			  <div class="form-group">
			    <label required="required" for="productimage1">Product Image</label>
			    <?php if(isset($r['thumb1']) & !empty($r['thumb1'])){ ?>
			    <br>
			    	<img src="<?php echo $r['thumb1'] ?>" widht="100px" height="100px">
			    	<a href="ajax/delprodimg1.php?id=<?php echo $r['id']; ?>">Delete Image</a>
			    <?php }else{ ?>
